...
def _get_physics_constants():
    ...
    return {
        'hbar': hbar, 'c': c, 'k_B': k_B,
        'MeV_to_J': MeV_to_J, 'MeV_to_kg': MeV_to_kg,
        'm_pi_MeV': m_pi_MeV, 'm_pi_J': m_pi_J, 'm_pi_kg': m_pi_kg,
        'N_cosmic': N_cosmic, 'g': g, 'hbar_c': hbar_c,
    }
...
def predict_independent() -> dict:
    ...
    results['alpha_w'] = {
        ...
        'notes': (f'Predicted 1/{1/aw_pred:.1f} vs observed 1/{1/aw_obs:.1f} at M_Z. '
    ...
    results['alpha_EM'] = {
        ...
        'notes': (f'With N_eff={N_eff_used:.0f}: alpha = {alpha_coherence:.3e} (off by ~{alpha_obs/alpha_coherence:.1e}×). '
    ...
    G_pred = (16.0 * np.pi ** 4 * p['hbar'] * p['c'] * alpha_obs
              / (p['m_pi_kg'] ** 2 * np.sqrt(p['N_cosmic'])))
    G_obs = PHYSICAL['G']
    # Also compute G with neutral pion mass for comparison
    m_pi_135_kg = 135.0 * p['MeV_to_kg']
    G_pred_135 = (16.0 * np.pi ** 4 * p['hbar'] * p['c'] * alpha_obs
                  / (m_pi_135_kg ** 2 * np.sqrt(p['N_cosmic'])))
    ...
    results['G'] = {
        ...
        'notes': (f'Uses observed α and m_π = {p["m_pi_MeV"]:.0f} MeV (charged pion). '
    ...
    gG_pred = p['c'] ** 3 * p['k_B'] / (p['hbar'] * G_obs)
    results['g_G'] = {
        'predicted': gG_pred,
        'observed': 2000.0,
        'error_pct': abs(gG_pred - 2000.0) / 2000.0 * 100,
        'status': 'APPROXIMATE',
        'formula': 'g_G = c³·k_B / (ħ·G)',
        'notes': f'Predicted {gG_pred:.1f} using observed G.',
    }
    ...
    results['Lambda'] = {
        ...
        'notes': (f'Prediction {Lambda_pred:.3e} vs observed {Lambda_obs:.1e} m⁻². '
...
def direct_solve(use_observed_alpha: bool = False) -> np.ndarray:
    ...
    G = (16.0 * np.pi ** 4 * p['hbar'] * p['c'] * alpha_EM
         / (p['m_pi_kg'] ** 2 * np.sqrt(p['N_cosmic'])))

    if G > 0 and np.isfinite(G):
        g_G = p['c'] ** 3 * p['k_B'] / (p['hbar'] * G)
    else:
        g_G = np.inf
...
def compute_constraints(x: np.ndarray) -> np.ndarray:
    ...
    G_target = (16.0 * np.pi ** 4 * p['hbar'] * p['c'] * alpha_EM
                / (p['m_pi_kg'] ** 2 * np.sqrt(p['N_cosmic'])))
    ...
    if G > 0 and np.isfinite(G):
        gG_target = p['c'] ** 3 * p['k_B'] / (p['hbar'] * G)
        F[6] = (g_G - gG_target) / max(abs(gG_target), 1.0)
    else:
        F[6] = g_G
...
def print_solution_table(x: np.ndarray = None) -> None:
    ...
    print("=" * 80)
    print(f"k_conscious = {FRAMEWORK['k_conscious']:.2f} "
          f"(derived from E(k) = k_BT at {FRAMEWORK['T_bio']}K)")
    print("-" * 80)
    print(f"{'Parameter':<12} {'Status':<12} {'Predicted':<16} "
          f"{'Observed':<16} {'Error':<10}")
    print("-" * 80)
