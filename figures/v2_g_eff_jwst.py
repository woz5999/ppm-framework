...
ax.plot(z_vals, g_eff_vals, color=GOLD, linewidth=2.5,
        label=r'PPM: $G_{\rm eff}(z) = (1+z)^{3/2}$', zorder=3)

# GR baseline
ax.axhline(1.0, color=GRAY, linewidth=2, linestyle='--', alpha=0.7,
           label=r'GR: $G_{\rm eff} = G_0$')

# JWST observational data (approximate excess factors from Labbé+ 2023, Boylan-Kolchin 2023)
# These represent how much more massive early galaxies are than ΛCDM predicts
# which maps to effective gravitational enhancement
z_obs = np.array([6.0, 7.5, 9.0, 10.5, 12.0])
excess_obs = np.array([3.5, 6.0, 10.0, 16.0, 25.0])  # approximate mass excess factors
excess_err = np.array([1.5, 2.5, 4.0, 6.0, 10.0])    # uncertainties

# Plot JWST data with error bars
ax.errorbar(z_obs, excess_obs, yerr=excess_err, fmt='o', color=CYAN, markersize=10,
            markeredgecolor=WHITE, markeredgewidth=1.5, capsize=5, capthick=2,
            elinewidth=2, label='JWST excess (approx.)', zorder=4)

# Fill between PPM curve and GR to show enhancement
ax.fill_between(z_vals, 1, g_eff_vals, alpha=0.1, color=GOLD)

ax.set_xlabel('Redshift $z$', fontsize=13, color=WHITE)
ax.set_ylabel(r'$G_{\rm eff}/G_0$ (or mass excess factor)', fontsize=13, color=WHITE)
